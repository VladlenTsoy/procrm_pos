...
class Products_model extends App_Model
{
    ...
    public function add_product($data)
    {
        ...
        if ($insert_id) {
            log_activity('New Product Added [ ID:'.$insert_id.', '.$data['name'].', Staff id '.get_staff_user_id().' ]');
            return $insert_id;
        }
        return false;
    }
    ...
    public function edit_product($data, $id)
    {
        ...
        if ($this->db->affected_rows() > 0) {
            ...
            log_activity('Product Details updated[ ID: '.$id.', '.$product->name.', Staff id '.get_staff_user_id().' ]');
        }
        if ($res) {
            return true;
        }
